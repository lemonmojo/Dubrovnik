//++Dubrovnik.CodeGenerator System_ReadOnlyMemoryA1.m
//
...
@implementation System_ReadOnlyMemoryA1
...
{
	return "System.ReadOnlyMemory`1";
}
...
{
	System_ReadOnlyMemoryA1 * object = [[self alloc] initWithSignature:"T[]" withNumArgs:1, [p1 monoRTInvokeObject]];
	return object;
}

+ (System_ReadOnlyMemoryA1 *)new_withArray:(System_Array *)p1 start:(int32_t)p2 length:(int32_t)p3
{
	System_ReadOnlyMemoryA1 * object = [[self alloc] initWithSignature:"T[],int,int" withNumArgs:3, [p1 monoRTInvokeObject], &p2, &p3];
	return object;
}
...
static System_ReadOnlyMemoryA1 * m_empty;
+ (System_ReadOnlyMemoryA1 *)empty
{
	...
	m_empty = [System_ReadOnlyMemoryA1 bestObjectWithMonoObject:monoObject];
...
- (System_ReadOnlySpanA1 *)span
{
	...
	_span = [System_ReadOnlySpanA1 bestObjectWithMonoObject:monoObject];
...
{
	[self invokeMonoMethod:"CopyTo(System.Memory`1<System.ReadOnlyMemory`1/T>)" withNumArgs:1, [p1 monoRTInvokeArg]];
}
...
{
	MonoObject *monoObject = [self invokeMonoMethod:"Equals(System.ReadOnlyMemory`1<System.ReadOnlyMemory`1/T>)" withNumArgs:1, [p1 monoRTInvokeArg]];
	return DB_UNBOX_BOOLEAN(monoObject);
}
...
{
	MonoObject *monoObject = [self invokeMonoMethod:"TryCopyTo(System.Memory`1<System.ReadOnlyMemory`1/T>)" withNumArgs:1, [p1 monoRTInvokeArg]];
	return DB_UNBOX_BOOLEAN(monoObject);
}
